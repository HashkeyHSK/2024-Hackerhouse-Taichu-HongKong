...
contract StablePool3 {
    // Number of tokens
    uint public constant N = 3;
    ...
    // 0.03%
    uint public constant SWAP_FEE = 300;
    ...
    uint public constant LIQUIDITY_FEE = (SWAP_FEE * N) / (4 * (N - 1));
    uint public constant FEE_DENOMINATOR = 1e6;

    address[N] public tokens;
    // Normalize each token to 18 decimals
    // Example - DAI (18 decimals), USDC (6 decimals), USDT (6 decimals)
    uint[N] private multipliers;
    ...
    constructor(address[N] memory _tokens) {
        tokens = _tokens;
        for (uint i = 0; i < N; i++) {
            multipliers[i] = 10 ** (18 - IERC20(_tokens[i]).decimals());
        }
    }
